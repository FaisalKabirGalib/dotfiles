...
function loadEnvKeys(): {
  ...
  try {
    const source = execSync(`bash -c 'source "${mcpEnvFile}" 2>/dev/null && echo "ZAI_WEB_SEARCH_KEY=$ZAI_WEB_SEARCH_KEY" && echo "REF_API_KEY=$REF_API_KEY" && echo "CONTEXT7_API_KEY=$CONTEXT7_API_KEY"'`, { encoding: "utf-8", timeout: 5000 });
    for (const line of source.split("\n")) {
      if (line.startsWith("ZAI_WEB_SEARCH_KEY=")) zaiWebSearchKey = line.split("=").slice(1).join("=");
    ...
    zaiWebSearchKey = process.env.ZAI_WEB_SEARCH_KEY || "";
    refApiKey = process.env.REF_API_KEY || "";
    context7ApiKey = process.env.CONTEXT7_API_KEY || "";
  }

  return { zaiWebSearchKey, refApiKey, context7ApiKey };
}
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    label: "Web Search",
    description:
      "Search the web for information. Returns titles, URLs, and summaries. Always use this FIRST for any search query. If results are insufficient, consider using library_docs or fetch_url as fallback.",
    promptSnippet: "Search the web for up-to-date information",
    promptGuidelines: [
      "Use web_search as the PRIMARY search tool for any query.",
      "If web_search results are empty or insufficient, use library_docs for library/framework documentation.",
    ...
    async execute(_toolCallId, params, _signal, onUpdate, ctx) {
      ...
      // 1. Try ZAI web search first
      if (keys.zaiWebSearchKey) {
        try {
          const session = await mcpInit(
            "https://api.z.ai/api/mcp/web_search_prime/mcp",
            { Authorization: `Bearer ${keys.zaiWebSearchKey}` },
          );
          ...
          // ZAI search failed, will try fallback below
        }
      }

      // 2. Fallback: Ref search if ZAI returned nothing
      if (results.length === 0 && keys.refApiKey) {
        try {
          const session = await mcpInit(
            "https://api.ref.tools/mcp",
            { "x-ref-api-key": keys.refApiKey },
          );
      ...
      const text = results.length > 0
        ? results.join("\n\n")
        : "No results found. Try using library_docs or fetch_url for more specific information.";
    ...
    async execute(_toolCallId, params, _signal, onUpdate, ctx) {
      ...
      if (!content && keys.refApiKey) {
        try {
          ...
          const raw = await mcpCall<string>(session, "ref_read_url", {
            url: params.url,
          }, ctx.signal);
          content = raw;
        } catch {
  ...
  pi.registerTool({
    ...
    label: "Library Docs",
    description:
      "Fetch up-to-date documentation for a library or framework. Use as fallback when web_search doesn't return sufficient results for library-specific queries. Supports GitHub-style owner/repo names.",
    ...
    async execute(_toolCallId, params, _signal, onUpdate, ctx) {
      ...
      if (keys.context7ApiKey) {
        try {
          const separator = topic || tokens ? "?" : "";
          const params_str = [
            topic ? `topic=${encodeURIComponent(topic)}` : "",
            tokens ? `tokens=${tokens}` : "",
          ].filter(Boolean).join("&");
          const url = `https://context7.com/api/v1/${lib}${separator}${params_str}`;
      ...
      return {
        content: [{ type: "text" as const, text: `No documentation found for ${lib}. Try web_search for broader results.` }],
