...
function resolveAgentModel(agent: AgentConfig): string {
  ...
  const m = piRef?.model;
  return m ? `${m.provider}/${m.id}` : "opencode-go/deepseek-v4-flash";
}
...
async function runSubagentWithFallback(
  ...
  let result = await runSubagent({ ...agent, model: resolvedModel }, task, cwd, signal, onUpdate);

  // Fallback: if deepseek model failed, retry with current pi model
  if (result.exitCode !== 0 && resolvedModel.startsWith("opencode-go/deepseek")) {
    const m = piRef?.model;
    const fallbackModel = m ? `${m.provider}/${m.id}` : undefined;
    if (fallbackModel && fallbackModel !== resolvedModel) {
      result = await runSubagent({ ...agent, model: fallbackModel }, task, cwd, signal, onUpdate);
    }
  }
...
export default function (pi: ExtensionAPI) {
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (params.chain && params.chain.length > 0) {
        ...
        for (let i = 0; i < chainSteps.length; i++) {
          ...
          const result = await runSubagentWithFallback(agent, task, cwd, signal, (progress) => {
      ...
      if (params.tasks && params.tasks.length > 0) {
        ...
        const results = await mapConcurrent(taskList, maxConcurrency, async (t, idx) => {
          ...
          const result = await runSubagentWithFallback(agent, t.task, t.cwd ?? cwd, signal, (progress) => {
      ...
      if (params.agent && params.task) {
        ...
        const result = await runSubagentWithFallback(agent, params.task, params.cwd ?? cwd, signal, (progress) => {
          liveResult.progress = progress;
          onUpdate?.({
  ...
  pi.registerCommand("run", {
    ...
    handler: async (args, ctx) => {
      ...
      const result = await runSubagentWithFallback(agent, task, ctx.cwd, new AbortController().signal);
