...
Item {
  ...
  readonly property string customCollectorPath: home + "/.config/omarchy/plugins/galib.agents/usage.py"
  ...
  function runUpdate(kind, agentIds) {
    if (!customUpdateProcess.running) {
      customUpdateProcess.command = ["python3", customCollectorPath]
      customUpdateProcess.running = true
    }
