...
Use PostgreSQL owned by the project through `DATABASE_URL`; do not introduce Neon, Vercel Postgres, Supabase, or another hosted database without an explicit request. When Drizzle is present, keep schema changes as reviewed migrations, generate before applying, and never apply a destructive migration without confirmation.
