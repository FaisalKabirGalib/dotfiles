...
local iface_handle = io.popen("route get default 2>/dev/null | awk '/interface:/{print $2}'")
local iface = (iface_handle:read("*a"):gsub("%s+$", ""))
iface_handle:close()
if iface == "" then
  iface = "en0"
end
...
sbar.exec("killall network_load >/dev/null 2>&1; " .. config_dir ..
  "/helpers/event_providers/network_load/bin/network_load " .. iface .. " network_update 2.0")
